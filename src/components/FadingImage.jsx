...
export const ImageFade = shaderMaterial(
  ...
  /*glsl*/ ` 
        ...
        float fbm(vec3 st) {
            // Create layers of noise at different scales
            float value = 0.0;
            float amplitude = 0.5;
            float frequency = 1.0;
...
const Fabric = ({ gridSize = 10 }) => {
    const meshRef = useRef();
    const [particles, springs] = useMemo(() => {
        let pts = [];
        let sprs = [];
        ...
        for (let i = 0; i < gridSize; i++) {
            for (let j = 0; j < gridSize; j++) {
                pts.push(new THREE.Vector3(i - gridSize / 2, j - gridSize / 2, 0));
            }
        }

        // Create springs (constraints) between adjacent particles
        for (let i = 0; i < gridSize - 1; i++) {
            for (let j = 0; j < gridSize - 1; j++) {
                sprs.push([i * gridSize + j, i * gridSize + j + 1]); // horizontal
                sprs.push([i * gridSize + j, (i + 1) * gridSize + j]); // vertical
            }
        }
    ...
    useFrame(() => {
        // Update particle positions here (physics simulation)
        // For example, apply simple gravity or other forces
...
export const FadingImage = (props) => {
  ...
  useFrame(() => {
    ...
    let shiftMagnitude = 0.01 * Math.sin(elapsedTime);
  ...
  return (
    <EffectComposer>
      ...
      <mesh
        ...
        receiveShadow
      >
        <planeGeometry ref={sphereRef} args={[4.0, 4.0, 4.0]} />
        <imageFade
